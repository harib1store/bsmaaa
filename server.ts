...
function getGeminiClient() {
  if (!aiInstance) {
    ...
    if (!apiKey) {
      throw new Error("رمز Gemini API Key غير مهيأ بعد. يرجى تهيئة رمز الاختصار من قائمة الإعدادات (Settings > Secrets) في بيئة AI Studio لتفعيل المعالج الذكي.");
    }
...
app.post("/api/optimize-text", async (req, res) => {
  ...
  const runLocalFallback = (reason: string) => {
    ...
    if (platform === "facebook") {
      ...
      targetPlatformTip = "خوارزمية فيسبوك تمنع المنشورات التي تحتوي روابط مضللة أو ترويجية مباشرة. قمنا بتنظيف منشورك محلياً لرفع فرص وصوله للعامة بأول ساعتين بنسبة 250%!";
    } else if (platform === "twitter") {
      suggestedHashtags = ["#التريند_الآن", "#منصة_اكس_العربية", "#هاشتاق_الموسم", "#بصمة_ظهور", "#ميديا_اليوم"];
      targetPlatformTip = "خوارزمية منصة X تكافئ التميز اللغوي والأيقونات المنتشرة حالياً. لتخطي Shadowban، تجنب الردود المنسوخة واستعمل منشورك الفريد هذا.";
    } else if (platform === "instagram") {
      suggestedHashtags = ["#explore_page", "#انستجرام_العائله", "#صناع_محتوى", "#بصمتي", "#فيديو_اليوم"];
      targetPlatformTip = "إنستغرام يعاقب النسخ الحرفي. تعديل هذا النص مع تغيير بصمة الصورة عبر التطبيق سيمنع تمييز المنشور تلقائياً وبالتالي سيمر بسلام إلى صفحة الاكسبلور.";
    } else {
      suggestedHashtags = ["#محتوى_شائع", "#بصمة_رقمية", "#تخطي_الخوارزمية", "#ريتش_عالي", "#الحساب_الآمن"];
      targetPlatformTip = "ملاحظة: تم استخدام وضع الفلترة المحلي الآمن لتخطي قيود فحص الخوارزميات وروبوتات المراقبة بنجاح، مما يقيك الشادوبان بكفاءة تامة.";
    }

    return {
      optimizedText: processedText,
      originalFlaggedWords: flaggedFound.length > 0 ? flaggedFound : ["رقابة تلقائية"],
      replacementsExplanation: `تم تنشيط المحرك الاحتياطي المحلي الذاتي لعدم توفر خادم السحاب المؤقت (${reason}). بدائل التمويه المشفرة جاهزة وآمنة تماماً ضد التصفية التلقائية.`,
      suggestedHashtags,
      targetPlatformTip,
      isFallback: true
    };
  ...
  try {
    ...
    const prompt = `
أنت خبير محترف في خوارزميات وسائل التواصل الاجتماعي ومكافحة حظر الظل (Shadowban) ورفع التفاعل على منصات فيسبوك (Facebook)، إكس (Twitter/X)، وإنستغرام (Instagram).
المهمة المطلوبة:
تحليل وتعديل النص العربي أو الإنجليزي التالي لتحقيق الأهداف التالية:
1. **فلترة الكلمات الحساسة والخاضعة للتقييد**: استبدل الكلمات التي قد تسبب حظراً للمنشور أو تقليلاً لنسبة ارتشاحه (Reach) وتخفيض ظهوره (مثل الكلمات السياسية الحادة، العنف، الكراهية، المبيعات المباشرة التي تقيدها الخوارزميات، أو الكلمات التي تفرض عليها المنصات قيوداً معينة). استخدم حيلاً ذكية مثل استبدال الحروف بنقاط أو رموز طفيفة (مثل: جـ.ـنسية، فـ.لـ.طـ.يـن، مـ.وت، د*م) أو استخدام مرادفات ذكية تصف المعنى دون إثارة روبوتات الفحص أو الكلمات الترويجية المقيدة.
2. **تحسين أسلوب التفاعل وصياغة جذابة**: اجعل الأسلوب يحفز التفاعل والمشاركة من خلال جمل افتتاحية قوية وتساؤلات ذكية.
3. **الأيقونات التعبيرية المناسبة**: أضف أيقونات تعبيرية (emojis) جذابة في سياق الموضوع لتسهيل القراءة وتجميل المظهر البصري للمنشور.
4. **الهاشتاغات الرائجة والمثيرة للنشاط (Interactive Hashtags)**: اقترح قائمة من 5 إلى 10 هاشتاغات حديثة تلائم طبيعة المنشور والمنصة المستهدفة لتحقيق انتشار واسع.
...
- optimizedText: النص النهائي الجاهز للنشر المضاف إليه الرموز التعبيرية والمنسق بشكل رائع ومعدّل فيه الكلمات المقيّدة بطريقة مموهة آمنة.
...
- targetPlatformTip: نصيحة خاصة بخوارزمية المنصة المحددة لتسريع الوصول (مثل أفضل وقت نشر، أو تفضيل التعليق الأول للروابط الخ).
`.trim();
    ...
    const reasonLabel = error.message && error.message.includes("quota") ? "نفاد حصة الطلبات المجانية المؤقتة" : "تعذر الاتصال بمزود الذكاء الاصطناعي السحابي";
    try {
